...
def parse_qstatgc():
    ...
    qstatgc_xml_out = Popen(["qstat", "-s", "r", "-xml"], stdout=PIPE).communicate()[0]
    tree = ET.ElementTree(ET.fromstring(qstatgc_xml_out))
    root = tree.getroot( )

    queue_xml_elements_total = root.findall("./cluster_queue_summary/total")

    total_slots = []

    for st in queue_xml_elements_total:
        qst = float(st)
        print qst
